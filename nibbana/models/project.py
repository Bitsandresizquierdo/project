...
class Project(models.Model):
    ...
    context = fields.Many2many(
        comodel_name='nibbana.context', compute='_get_context',
        inverse='_set_context', search='_search_context')
    ...
    tasks = fields.One2many(comodel_name='nibbana.task',
                            inverse_name='project')
    today_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Today')])
    tomorrow_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Tomorrow')])
    next_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Next')])
    someday_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Someday')])
    waiting_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Waiting')])
    scheduled_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Scheduled')])
    done_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Done')])
    cancelled_tasks = fields.One2many(
        comodel_name='nibbana.task', inverse_name='project',
        context={'active_test': False}, domain=[('state', '=', 'Cancelled')])
    references = fields.One2many(comodel_name='nibbana.reference',
                                 inverse_name='project')
    reference_count = fields.Integer(compute='_reference_count')
    focus = fields.Selection(selection=[
        ('0', _('Non-focused')),
        ('1', _('Focused'))], default='0')
    uid = fields.Char(required=True, index=True, size=32, default=generate_uid)
    ...
    def write(self, vals):
        for rec in self:
            if not vals.get('state_change_count') and (
                    'state' in vals or 'focus' in vals):
            ...
            if vals.get('state'):
                # Do metrics
                increment_metric(
                    rec, code='projects_changed', name=_('Projects'))
                if vals['state'] == 'Done' and rec.state != 'Done':
                    # Project set to Done
                    increment_metric(
                        rec, code='projects_done', name=_('Projects'))
                ...
                if vals['state'] in ['Done', 'Cancelled']:
                    ...
                    vals['active'] = True
                    for t in rec.with_context(
                            active_test=False).mapped('tasks'):
                        if t.state not in ['Done', 'Cancelled']:
                            t.write({'active': True})
                    rec.with_context(
                        active_test=False).mapped(
                        'references').write({'active': True})
            self.env['nibbana.timeline'].timeline_update_event(
                rec, vals, ['state', 'name', 'note', 'description', 'focus'])
    ...
    def toggle_focus(self):
        for rec in self:
            rec.focus = '1' if rec.focus == '0' else '0'
        if not self.env.context.get('group_by'):
            self.env['bus.bus'].sendone('nibbana_tree_reload', 'reload')

    def _open_task_count(self):
        for rec in self:
            rec.open_task_count = self.with_context(
    ...
    def _closed_task_count(self):
        for rec in self:
            rec.closed_task_count = self.with_context(
                {'active_test': False}).env['nibbana.task'].search_count([
                    ('project', '=', rec.id),
    ...
    def _reference_count(self):
        for rec in self:
            rec.reference_count = self.with_context(
                {'active_test': False}).env['nibbana.reference'].search_count(
                [('project', '=', rec.id)])

    def _get_context(self):
        for rec in self:
            rec.context = [k.context_id.id for k in self.env[
    ...
    def _search_context(self, operator, value):
        if operator == 'ilike':
            ...
            raise ValidationError(
                _('Search context by {} not implemented!').format(operator))
    ...
    def _get_area(self):
        for rec in self:
            ...
            rec.area = area.area_id if area else False
    ...
    def _has_context(self):
        for rec in self:
            if rec.context:
                rec.has_context = True
            else:
                rec.has_context = False


    def _context_list(self):
        for rec in self:
            rec.context_list = ', '.join([k.name for k in rec.context])
