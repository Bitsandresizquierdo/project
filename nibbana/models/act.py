...
class Act(models.Model):
    ...
    def _get_context_list(self):
        for rec in self:
            rec.context_list = rec.task_id.context_list if rec.task_id else \
                rec.project_id.context_list

    def _get_area(self):
        for rec in self:
            rec.area = rec.task_id.area if rec.task_id else rec.project_id.area
    
    def _get_context(self):
        for rec in self:
            rec.context = rec.task_id.context if rec.task_id else rec.project_id.context
